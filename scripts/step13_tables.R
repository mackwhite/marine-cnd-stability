...
###author(s): Mack White
###goal(s): generate summary tables for the paper
...
librarian::shelf(tidyverse, readxl, MuMIn, sjPlot, lme4, corrplot, splitstackshape,
...
### test for total number of ind fish measured
inds <- rbind(dt_mutate_1, dt_mutate_fce) #1,462,614
rm(dt_mutate,dt_mutate_1,fce_plus_zero_save,no_fce_or_zeros)

spp_num <- inds |> select(scientific_name) |> distinct() #538 species
##########################################################################
