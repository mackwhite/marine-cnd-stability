...
map_dt <- read_csv("../cndwg_website/data/LTER_Site_Coords_w_information.csv") |> 
      filter(Site %in% c("FCE", "MCR", "SBC", "VCR", "CCE", "NGA", "PIE")) |> 
      rename(program = Site,
             lat = Latitude,
             long = Longitude) |> 
...
palette <- c("Overall"="#000000", "FCE"="#64a988", "MCR"="#ff967d", 'PCCC'="#2A788EFF", "PCCS"="#8b6b93",
             'SBC'='#ff3f4c', "VCR"="#9b9254", "PIE"="yellow", "CCE"="pink", "NGA"="white")
...
site_map <- get_map(
      ...
      c(left = -188.7, bottom = -20.0, right = -24.9, top = 65.0),
      maptype = 'satellite',
      source = 'google',
      api_key = api_secret
)
...
ggsave("output/ms-second-round/plots/presentation-map.tiff", units = "in", width = 10,
       height = 8, dpi =  600, compression = "lzw")
ggsave("output/ms-second-round/plots/presentation-map.svg", units = "in", width = 10,
       height = 8, dpi =  600)
